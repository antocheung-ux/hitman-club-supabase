/**
 * Hitman Pekanbaru Hashing Club - App Logic
 * Menggunakan window.sb dari config.js
 */
(function() {
    // Langsung pakai window.sb, tidak perlu redeclare
    const sb = window.sb;

    if (!sb) {
        console.error('❌ window.sb tidak tersedia. Cek config.js');
        alert('Database belum siap. Refresh halaman.');
        return;
    }

    if (!sb.auth) {
        console.error('❌ sb.auth tidak tersedia. Cek anon key di config.js');
        return;
    }

    console.log('✅ App.js initialized with window.sb');
